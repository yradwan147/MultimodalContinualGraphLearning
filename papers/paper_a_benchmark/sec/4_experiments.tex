...
\begin{itemize}
    ...
    \item \textbf{EWC}~\citep{Kirkpatrick2017}: Elastic Weight Consolidation penalizes changes to parameters important for previous tasks via the Fisher information matrix ($\lambda = 400$).
...
All KGE-based methods use embedding dimension $d = 128$, learning rate $\eta = 0.001$, batch size 512, and are trained for 100 epochs per task with early stopping (patience 10).
We use the Adam optimizer with weight decay $10^{-5}$.
For EWC, $\lambda = 400$; for experience replay, the buffer stores 10\% of each task's training triples.
CMKL uses a 4-head cross-attention fusion module, modality-specific Fisher matrices, and K-means diverse selection for the multimodal memory buffer.
...
The RAG agent using Qwen2.5-7B achieves an overall accuracy of 47.3\% on continual KGQA, with Exact Match\,=\,31.2\% and token-level F1\,=\,52.8\%.
Performance is stable across temporal snapshots due to the non-parametric retrieval mechanism: answers are generated from the current KG snapshot rather than memorized parameters.
Questions about newly added entities in $t_1$ are answered with 43.1\% accuracy, compared to 51.7\% for entities present in both snapshots, reflecting the challenge of sparse context for newly introduced knowledge.
